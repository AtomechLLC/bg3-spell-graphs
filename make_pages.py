...
def wrap(src, dst, emoji):
    ...
    home = ('<a href="index.html" id="suitehome" title="All spell graphs" '
            ...
            '<script>function sgMark(){try{localStorage.setItem("sg_last",JSON.stringify('
            '{u:location.pathname.split("/").pop()+location.hash,t:document.title}))}catch(e){}}'
            'sgMark();addEventListener("hashchange",sgMark);</script>')
    html = (f'<!doctype html>\n<html lang="en">\n<head>\n<meta charset="utf-8">\n'
...
#resume{{display:none;margin:-38px 0 44px;font:400 12px "IBM Plex Mono",monospace;color:var(--muted)}}
#resume a{{color:var(--gold-ink);text-decoration:none}}
#resume a:hover{{text-decoration:underline}}
.fsc{{display:inline-flex;align-items:center;gap:7px;border:1px solid var(--line);
  border-radius:999px;padding:5px 14px;font:500 11px "IBM Plex Mono",monospace;color:var(--muted);
  text-decoration:none;letter-spacing:.05em;margin-bottom:30px}}
.fsc:hover{{border-color:var(--gold);color:var(--gold-ink)}}
...
<script>
try {{
  const l = JSON.parse(localStorage.getItem('sg_last') || 'null');
  if (l && l.u && l.u.indexOf('index') !== 0) {{
    const d = document.getElementById('resume');
    const a = document.createElement('a');
    a.href = encodeURI(l.u);
    a.textContent = l.t;
    d.append('continue where you left off: ', a);
