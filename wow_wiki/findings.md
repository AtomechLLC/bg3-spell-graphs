# The Identical-Spell List

Which WoW Classic abilities are mostly the same ability. Of **423 distinct trainer-taught class abilities**, **202 (48%) fall into 25 families** — by far the highest of the three games studied, because classic WoW reskins along an axis the others barely use: **across classes**.

- <span class="tier tier-clone">Verbatim clone</span> — same sentence, one noun swapped (often into a different class's book).
- <span class="tier tier-template">Shared template</span> — one chassis, many payloads (totems, blessings, curses).
- <span class="tier tier-engine">Shared engine</span> — same underlying design in class-flavored prose.

| Family | Tier | Size | Peak sim | What actually differs |
|---|---|---|---|---|
| [[families/conjured|💎 The Conjured Commissary]] | <span class="tier tier-clone">Verbatim clone</span> | 23 abilities · 2 classes | 1.00 | the item conjured; the tier in parentheses |
| [[families/cat-is-rogue|🐱 The Druid Costume Shop]] | <span class="tier tier-clone">Verbatim clone</span> | 13 abilities · 4 classes | 1.00 | which body performs it |
| [[families/teleports|🌀 The Hearth Network]] | <span class="tier tier-clone">Verbatim clone</span> | 12 abilities · 1 classes | 1.00 | the destination city; self vs. group |
| [[families/tracking|🐾 The Tracking Dial]] | <span class="tier tier-clone">Verbatim clone</span> | 11 abilities · 4 classes | 1.00 | the creature type on the minimap |
| [[families/cleanses|🧼 The Cleanse Counter]] | <span class="tier tier-clone">Verbatim clone</span> | 11 abilities · 5 classes | 1.00 | the debuff type removed — Cure Poison ships in two class books verbatim |
| [[families/heals|❤️‍🩹 One Heal, Nine Names]] | <span class="tier tier-clone">Verbatim clone</span> | 9 abilities · 4 classes | 1.00 | class flavor, cast speed, and efficiency knobs |
| [[families/rez|⚰️ The Resurrection Union]] | <span class="tier tier-clone">Verbatim clone</span> | 4 abilities · 4 classes | 1.00 | the class label and the cooldown |
| [[families/interrupts|✋ The Interrupt Union]] | <span class="tier tier-clone">Verbatim clone</span> | 4 abilities · 3 classes | 0.88 | the limb or element used |
| [[families/stings|🦂 The Sting Clip]] | <span class="tier tier-clone">Verbatim clone</span> | 3 abilities · 1 classes | 0.89 | the payload injected |
| [[families/shocks|⚡ The Shock Battery]] | <span class="tier tier-clone">Verbatim clone</span> | 3 abilities · 1 classes | 0.58 | element and rider |
| [[families/poisons|🧪 The Numbered Vials]] | <span class="tier tier-clone">Verbatim clone</span> | 3 abilities · 1 classes | 1.00 | the roman numeral |
| [[families/mirror|🪞 Amplify & Dampen]] | <span class="tier tier-clone">Verbatim clone</span> | 2 abilities · 1 classes | 0.91 | the sign |
| [[families/polymorph|🐑 The Polymorph Barn]] | <span class="tier tier-clone">Verbatim clone</span> | 2 abilities · 1 classes | 0.96 | the barnyard animal |
| [[families/totems|🗿 The Totem Foundry]] | <span class="tier tier-template">Shared template</span> | 21 abilities · 1 classes | 1.00 | the payload planted in the ground |
| [[families/group-ladder|🙌 The Greater Ladder]] | <span class="tier tier-template">Shared template</span> | 10 abilities · 3 classes | 1.00 | single target vs. whole party; reagent cost |
| [[families/protection|🧿 The Protection Rack]] | <span class="tier tier-template">Shared template</span> | 10 abilities · 3 classes | 1.00 | how the damage is refused: armor, absorb, or school ward |
| [[families/summons|😈 The Stable & The Circle]] | <span class="tier tier-template">Shared template</span> | 8 abilities · 2 classes | 1.00 | who answers the call |
| [[families/blessings|⚖️ The Blessing Rack]] | <span class="tier tier-template">Shared template</span> | 7 abilities · 1 classes | 0.91 | the stat blessed |
| [[families/auras|🛡️ The Aura Carousel]] | <span class="tier tier-template">Shared template</span> | 7 abilities · 1 classes | 1.00 | the passive granted to the party |
| [[families/curses|💀 The Curse Catalogue]] | <span class="tier tier-template">Shared template</span> | 7 abilities · 1 classes | 1.00 | the affliction applied |
| [[families/aspects|🦅 The Aspect Dial]] | <span class="tier tier-template">Shared template</span> | 6 abilities · 1 classes | 0.83 | the animal and its bonus |
| [[families/fears|😱 The Fear Franchise]] | <span class="tier tier-template">Shared template</span> | 5 abilities · 4 classes | 0.83 | targets, count, and radius |
| [[families/seals|✝️ The Seal Press]] | <span class="tier tier-template">Shared template</span> | 5 abilities · 1 classes | 0.87 | the on-hit payload |
| [[families/bolts|🏹 The Bolt Engine]] | <span class="tier tier-engine">Shared engine</span> | 10 abilities · 5 classes | 1.00 | school, speed, and rider |
| [[families/dots|🩸 The Affliction Engine]] | <span class="tier tier-engine">Shared engine</span> | 8 abilities · 6 classes | 1.00 | school, duration, and delivery |

## Top measured pairs

| Similarity | Ability A | Ability B |
|---|---|---|
| `1.000` | Growl (Druid) | Taunt (Warrior) |
| `1.000` | Cure Poison (Druid) | Cure Poison (Shaman) |
| `1.000` | Challenging Roar (Druid) | Challenging Shout (Warrior) |
| `1.000` | Feline Grace (Druid) | Safe Fall (Rogue) |
| `1.000` | Bash (Druid) | Hammer of Justice (Paladin) |
| `1.000` | Faerie Fire (Druid) | Faerie Fire (Feral) (Druid) |
| `1.000` | Soothe Animal (Druid) | Mind Soothe (Priest) |
| `1.000` | Wrath (Druid) | Starfire (Druid) |
| `1.000` | Moonfire (Druid) | Immolate (Warlock) |
| `1.000` | Healing Touch (Druid) | Flash of Light (Paladin) |
| `1.000` | Healing Touch (Druid) | Holy Light (Paladin) |
| `1.000` | Healing Touch (Druid) | Flash Heal (Priest) |
| `1.000` | Healing Touch (Druid) | Healing Wave (Shaman) |
| `1.000` | Healing Touch (Druid) | Lesser Healing Wave (Shaman) |
| `1.000` | Track Beasts (Hunter) | Track Humanoids (Hunter) |
| `1.000` | Track Beasts (Hunter) | Track Undead (Hunter) |
| `1.000` | Track Beasts (Hunter) | Track Elementals (Hunter) |
| `1.000` | Track Beasts (Hunter) | Track Demons (Hunter) |
| `1.000` | Track Beasts (Hunter) | Track Giants (Hunter) |
| `1.000` | Track Beasts (Hunter) | Track Dragonkin (Hunter) |

Scores are masked-tooltip similarity ([[methodology|method]]); classic tooltips are macro-parameterized in the game data itself, so the masking mostly just removes what Blizzard already treats as a variable.

---
*Linked from: [[classes/druid|Druid]] · [[classes/hunter|Hunter]] · [[classes/mage|Mage]] · [[classes/paladin|Paladin]] · [[classes/priest|Priest]] · [[classes/rogue|Rogue]] · [[classes/shaman|Shaman]] · [[classes/warlock|Warlock]] · [[classes/warrior|Warrior]] · [[families/aspects|The Aspect Dial]] · [[families/auras|The Aura Carousel]] · [[families/blessings|The Blessing Rack]] · [[families/bolts|The Bolt Engine]] · [[families/cat-is-rogue|The Druid Costume Shop]] · [[families/cleanses|The Cleanse Counter]] · [[families/conjured|The Conjured Commissary]] · [[families/curses|The Curse Catalogue]] · [[families/dots|The Affliction Engine]] · [[families/fears|The Fear Franchise]] · [[families/group-ladder|The Greater Ladder]] · [[families/heals|One Heal, Nine Names]] · [[families/interrupts|The Interrupt Union]] · [[families/mirror|Amplify & Dampen]] · [[families/poisons|The Numbered Vials]] · [[families/polymorph|The Polymorph Barn]] · [[families/protection|The Protection Rack]] · [[families/rez|The Resurrection Union]] · [[families/seals|The Seal Press]] · [[families/shocks|The Shock Battery]] · [[families/stings|The Sting Clip]] · [[families/summons|The Stable & The Circle]] · [[families/teleports|The Hearth Network]] · [[families/totems|The Totem Foundry]] · [[families/tracking|The Tracking Dial]] · [[overview|Overview]] · [[spells|All Abilities, Tagged]]*
