...
fam("forms", "The Form Rack", TEMPLATE, "🐻",
    ...
    "the body worn",
    "Every druid form is the identical two-aura package: `Shapeshift (X)` plus `Immunity - Mechanic (Polymorphed)`. The blended similarity clusters all of them above 0.78 — one costume engine, six costumes.",
    "The form, its stat payload, and which kit it unlocks.",
    "The chassis that makes [[families/cat-is-rogue|the Costume Shop]] possible — the forms are one design; the abilities they unlock are the photocopies."),
fam("detect-invis", "The Invisibility Arms Race", CLONE, "🔦",
    ["Detect Lesser Invisibility", "Detect Invisibility", "Detect Greater Invisibility"],
    "the tier of invisibility pierced",
    "Three spellbook lines, one `Mod Invisibility Detection` aura at three magnitudes — a rank ladder promoted into names, like the numbered poisons.",
    "The tier number hiding in the adjective.",
    "The counter-half of an arms race whose other half (invisibility potions and effects) lives outside the trainer book."),
...
fam("stuns", "The Stun Union", CLONE, "💫",
    ["Bash", "Hammer of Justice", "Cheap Shot", "Kidney Shot"],
    "the limb, hammer, or ambush delivering it",
    "All four apply the same `Stun` aura: Bash and Hammer of Justice measure ≥ 0.78 as the bear/paladin cross-class pair; Cheap Shot and Kidney Shot are the Rogue's stealth-gated and combo-spent versions.",
    "The class, the resource, and the gate (stealth, combo points, cooldown).",
    "Like [[families/interrupts|the Interrupt Union]] — every melee class bought the same button."),
fam("sleepers", "The Sleepers", CLONE, "😴",
    ["Hibernate", "Wyvern Sting", "Sap"],
    "the lullaby's delivery and legal targets",
    "Effect data files all three as the same incapacitate aura: Hibernate and Wyvern Sting cluster at ≥ 0.78 (beasts-only sleep, cast vs. sting-delivered); Sap is the stealth-opener version for humanoids.",
    "Target type, delivery, and the damage-breaks clause.",
    "WoW's Hold Person/Hold Monster split — the target-type parameter sold as separate class abilities."),
fam("static-shell", "The Static Shell", CLONE, "🌩️",
    ["Lightning Shield", "Shadowguard", "Thorns"],
...
fam("breath", "The Deep Breath Pair", CLONE, "🫧",
    ["Unending Breath", "Water Breathing"],
    "nothing but the class selling it",
    "One `Underwater Breathing` aura, two class books (Warlock and Shaman) — a verbatim cross-class SKU like BG3's triple Shield.",
    "The name and the reagent.",
    "The smallest possible reskin: same aura, same duration slot, different vendor."),
...
fam("soothe", "The Soothe Pair", CLONE, "🕊️",
    ["Mind Soothe", "Soothe Animal"],
    "humanoid vs. beast",
    "Both apply `Mod Detect Range` (aura 91) — shrinking how far the target notices you. Same aura, two creature types, two classes (Priest and Druid).",
    "The legal target type and the class stamp.",
    "The WoW twin of D&D's Charm Person / Animal Friendship split — and only the effect data shows they are the same aura."),
...
fam("cat-is-rogue", "The Druid Costume Shop", CLONE, "🐱",
    ["Stealth", "Prowl", "Ambush", "Ravage", "Backstab", "Shred", "Cower", "Feint",
     "Taunt", "Growl", "Challenging Shout", "Challenging Roar", "Feline Grace", "Safe Fall",
     "Sprint", "Dash", "Faerie Fire", "Faerie Fire (Feral)"],
    "which body performs it",
    "Seven rogue/warrior abilities, photocopied into the Druid's cat and bear forms: Stealth→Prowl, Ambush→Ravage, Backstab→Shred, Feint→Cower, Taunt→Growl, Challenging Shout→Challenging Roar — and, revealed by the effect data, Safe Fall→Feline Grace (both are the literal `Safe Fall` aura, id 144). Every pair measures 0.8–0.95.",
...
fam("fears", "The Fear Franchise", TEMPLATE, "😱",
    ["Fear", "Psychic Scream", "Intimidating Shout", "Scare Beast", "Howl of Terror", "Turn Undead"],
